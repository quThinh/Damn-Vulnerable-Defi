...
contract TheRewarderChallenge is Test {
    ...
    function test_theRewarder() public checkSolvedByPlayer {
        uint256 maxDVTClaim = TOTAL_DVT_DISTRIBUTION_AMOUNT / 11524763827831882;
        uint256 maxWETHClaim = TOTAL_WETH_DISTRIBUTION_AMOUNT / 1171088749244340;
        bytes32[] memory dvtLeaves = _loadRewards(
            "/test/the-rewarder/dvt-distribution.json"
        );
        bytes32[] memory wethLeaves = _loadRewards(
        ...
        console.log(maxDVTClaim);
        console.log(maxWETHClaim);
        Claim[] memory claims = new Claim[](maxDVTClaim + maxWETHClaim);
        ...
        for (uint256 i = 0; i < maxWETHClaim; i++) {
            claims[i + maxDVTClaim] = Claim({
                batchNumber: 0,
